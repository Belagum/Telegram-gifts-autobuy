...
class SQLAlchemyAccountRepository:
    ...
    def get_api_credentials(
        ...
        credentials = ApiCredentials(
            api_id=api_profile.api_id, api_hash=api_profile.api_hash
        )
        ...
        try:
            ...
            if not account:
                account = self._create_new_account(
                    user_id, api_profile_id, phone, session_path, telegram_id
                )
            ...
            raise RepositoryError(
                f"Failed to save account: {e}", error_code="save_account_failed"
            ) from e

    def _find_existing_account(
        self, user_id: int, phone: str, telegram_id: int | None
    ) -> Account | None:
        account = (
    ...
    def _update_account_data(
        self, account: Account, account_data: AccountData, session_path: str
    ) -> None:
    ...
    def _commit_account(
        self, account: Account, user_id: int, phone: str, telegram_id: int | None
    ) -> None:
        try:
            self._db.commit()
            logger.info(
    ...
    def _retry_update_existing_account(
        self, telegram_id: int, source_account: Account
    ) -> None:
