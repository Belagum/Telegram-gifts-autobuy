...
def register():
    ...
    try:
        ...
        logger.exception(f"auth.register: error (req_id={req_id}, username='{locals().get('u') or ''}')")
...
def login():
    ...
    try:
        ...
        logger.exception(f"auth.login: error (req_id={req_id}, username='{locals().get('u') or ''}')")
        return jsonify({"error": "internal_error"}), 500
    finally:
        try:
            db_gen.close()
...
def logout(db: Session):
    ...
    try:
        ...
        logger.info(
            f"auth.logout: start (req_id={req_id}, user_id={getattr(request, 'user_id', None)}, token='{token_hint}')"
        )
        if token:
        ...
        dt = (perf_counter() - t0) * 1000
        logger.info(
            f"auth.logout: ok (req_id={req_id}, user_id={getattr(request, 'user_id', None)}, dt_ms={dt:.0f})"
        )
        resp = make_response(jsonify({"ok": True}))
        resp.delete_cookie("auth_token")
        return resp
    except Exception:
        logger.exception(f"auth.logout: error (req_id={req_id}, user_id={getattr(request, 'user_id', None)})")
        return jsonify({"error": "internal_error"}), 500
