...
from backend.infrastructure.telegram_auth.telegram_error_mapper import map_telegram_error
...
class PyrogramClientWrapper:
    ...
    def connect_and_send_code(self, phone: str) -> tuple[Any, str]:
        client, code_hash = self._event_loop.run(
            self._connect_and_send_code(phone)
        )
        self._client = client
    ...
    def sign_in_with_code(
        ...
        try:
            self._event_loop.run(
                self._sign_in(client, phone, phone_code_hash, code)
            )
