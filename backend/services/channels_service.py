...
def _norm_ch_id(v)->int:
    ...
    if s.startswith("-100"):
        tail=re.sub(r"\D","",s[4:])
        if not tail: raise ValueError("invalid channel id")
        return int("-100"+tail)
    digits=re.sub(r"\D","",s)
    if not digits: raise ValueError("invalid channel id")
...
def _probe_one_account(acc:Account, ch_id:int)->tuple[str|None,bool]:
    async def work(timeout:float)->tuple[str|None,bool]:
        title=None; joined=False
        m=await _safe_tg_call(acc, lambda c: c.get_chat_member(ch_id,"me"), timeout)
        if m is not None:
            try: joined=_status_joined(getattr(m,"status",None))
            except Exception: joined=False
            try:
                chat_obj=getattr(m,"chat",None)
                if chat_obj and getattr(chat_obj,"title",None): title=chat_obj.title
            except Exception: pass
        if not title:
            chat=await _safe_tg_call(acc, lambda c: c.get_chat(ch_id), timeout)
            if chat is not None and getattr(chat,"title",None): title=chat.title
        return title, joined
    t0=perf_counter()
    try:
...
def create_channel(db:Session, user_id:int, channel_id, price_min, price_max, supply_min, supply_max, title_input:str|None)->dict:
    ...
    except ValueError as e: return {"error":"bad_range","detail":str(e)}
    exists=(db.query(Channel.id).filter(Channel.user_id==user_id, Channel.channel_id==ch_id).first())
    if exists: return {"error":"duplicate_channel"}
    probed_title, joined=_probe_any_account(db, user_id, ch_id)
    if not joined: return {"error":"not_joined","detail":"в канал не войдено ни на каком аккаунте"}
...
def update_channel(db:Session, user_id:int, ch_id:int, **f)->dict:
    ...
    if not ch: return {"error":"not_found"}
    if "title" in f: f["title"]=(f["title"] or "").strip()
    if "price_min" in f or "price_max" in f:
        try: lo,hi=_coerce_range("price_min","price_max", f.get("price_min", ch.price_min), f.get("price_max", ch.price_max))
        except ValueError as e: return {"error":"bad_range","detail":str(e)}
        f["price_min"], f["price_max"]=lo,hi
    if "supply_min" in f or "supply_max" in f:
        try: lo,hi=_coerce_range("supply_min","supply_max", f.get("supply_min", ch.supply_min), f.get("supply_max", ch.supply_max))
        except ValueError as e: return {"error":"bad_range","detail":str(e)}
        f["supply_min"], f["supply_max"]=lo,hi
    for k in ("title","price_min","price_max","supply_min","supply_max"):
        if k in f: setattr(ch, k, f[k])
    db.commit()
    return {"ok": True}
