...
from backend.infrastructure.telegram_auth.exceptions import (EventLoopError,
                                                             TelegramAuthError)
...
class EventLoopManager:
    def __init__(self) -> None:
        ...
        self._thread: threading.Thread = threading.Thread(
            target=self._run_loop, daemon=True, name="TelegramAuthEventLoop"
        )
        self._started = False
    ...
    def run(self, coro: Coroutine[Any, Any, Any]) -> Any:
        if not self._started:
            raise EventLoopError(
                "Event loop not started", error_code="loop_not_started"
            )

        logger.debug(
            f"EventLoopManager: scheduling coroutine thread={self._thread.name}"
        )
        ...
        try:
            ...
            logger.exception(
                f"EventLoopManager: execution failed thread={self._thread.name}"
            )
            raise EventLoopError(
                f"Failed to execute coroutine: {e}", error_code="execution_failed"
            ) from e
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logger.exception(
                f"EventLoopManager: stop failed thread={self._thread.name}"
            )
