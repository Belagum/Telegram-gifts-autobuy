...
from backend.interfaces.http.dto.admin import (
    ActionCategoriesDTO,
    AuditLogDTO,
    AuditLogListDTO,
    AuditLogsFilterDTO,
...
class AdminController:
    ...
    def audit_logs(self) -> tuple[Response, int]:
        ...
        try:
            ...
            if debug_mode:
                logger.warning(f"admin.audit_logs validation error for user={user_id}: {exc}")
    ...
    def action_categories(self) -> tuple[Response, int]:
        ...
        try:
            ...
            if debug_mode:
                logger.exception(f"admin.action_categories failed for user={user_id}: {exc}")
            else:
    ...
    def suspicious_activity(self) -> tuple[Response, int]:
        ...
        try:
            ...
            return jsonify({"activities": [dto.model_dump() for dto in activity_dtos]}), 200
        except Exception as exc:
            if debug_mode:
                logger.exception(f"admin.suspicious_activity failed for user={user_id}: {exc}")
            else:
    ...
    def dashboard_stats(self) -> tuple[Response, int]:
        ...
        try:
            ...
            if debug_mode:
                logger.info(f"admin.dashboard_stats completed successfully for user={user_id}")
            else:
            ...
            if debug_mode:
                logger.exception(f"admin.dashboard_stats failed for user={user_id}: {exc}")
            else:
    ...
    def as_blueprint(self) -> Blueprint:
        ...
        bp.add_url_rule("/users/<int:user_id>/unlock", view_func=self.unlock_user, methods=["POST"])
        bp.add_url_rule("/dashboard-stats", view_func=self.dashboard_stats, methods=["GET"])
