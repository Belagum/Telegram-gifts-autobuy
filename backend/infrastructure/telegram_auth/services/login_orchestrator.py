...
from backend.infrastructure.telegram_auth.exceptions import (
    ApiProfileNotFoundError, LoginError, LoginNotFoundError)
...
class PyroLoginManager:
    ...
    def start_login(self, user_id: int, api_profile_id: int, phone: str) -> LoginResult:
        logger.info(
        ...
        try:
            ...
            wrapper = PyrogramClientWrapper(session_path, credentials, self._event_loop)
            ...
            if is_telegram_error:
                self._cleanup_on_error(
                    None, session_path if "session_path" in locals() else None
                )
            return LoginResult.fail(
            ...
            logger.exception(
                f"PyroLoginManager: start_login unexpected error user_id={user_id}"
            )
            self._cleanup_on_error(
                None, session_path if "session_path" in locals() else None
            )
    ...
    def confirm_code(self, login_id: str, code: str) -> LoginResult:
        ...
        try:
            ...
            wrapper: PyrogramClientWrapper | None = session._wrapper

            if client is None or wrapper is None:
                raise LoginError(
                    "Session state corrupted", error_code="session_corrupted"
                )

            success = wrapper.sign_in_with_code(
                client, session.phone, session.phone_code_hash or "", code
            )
            ...
            if not is_non_critical:
                removed_session = self._session_manager.remove_session(login_id)
                if removed_session:
                    self._cleanup_on_error(
                        removed_session, removed_session.session_path
                    )
            ...
            removed_session = self._session_manager.remove_session(login_id)
            if removed_session:
                self._cleanup_on_error(removed_session, removed_session.session_path)
            return LoginResult.fail(
                error="unexpected",
    ...
    def confirm_password(self, login_id: str, password: str) -> LoginResult:
        ...
        try:
            ...
            logger.info(
                f"PyroLoginManager: confirm_password success login_id={login_id}"
            )
    ...
    def cancel(self, login_id: str) -> LoginResult:
        ...
        if wrapper and client:
            self._cleanup_session(login_id, session, wrapper, client, purge_files=True)
    ...
    def _cleanup_session(
        ...
        try:
            ...
            logger.exception(
                f"PyroLoginManager: failed to disconnect client login_id={login_id}"
            )
    ...
    def _cleanup_on_error(
        self, session: LoginSession | None, session_path: str | None
    ) -> None:
        if session:
            client = getattr(session, "_client", None)
            wrapper: PyrogramClientWrapper | None = getattr(session, "_wrapper", None)
            ...
            if wrapper and client:
                try:
                    wrapper.disconnect(client)
                    logger.debug(
                        "PyroLoginManager: client disconnected during cleanup_on_error"
                    )
                except Exception:
                    logger.debug(
        ...
        if session_path:
            try:
                ...
                logger.exception(
                    f"PyroLoginManager: cleanup_on_error failed path={session_path}"
                )
