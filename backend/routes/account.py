...
def accounts(db: Session):
    ...
    try:
        ...
        logger.info(f"accounts.list: {'ready' if code == 200 else 'refreshing'} (user_id={uid}, dt_ms={dt:.0f})")
        return resp, code
    except Exception:
...
def create_apiprofile(db: Session):
    ...
    try:
        ...
        if exist_id:
            logger.warning(f"apiprofile.create: duplicate api_id (user_id={uid}, existing_id={exist_id.id})")
        ...
        if exist_hash:
            logger.warning(f"apiprofile.create: duplicate api_hash (user_id={uid}, existing_id={exist_hash.id})")
...
def send_code(db: Session):
    ...
    try:
        ...
        if exist:
            logger.warning(
                f"auth.send_code: phone_exists (user_id={uid}, acc_id={exist.id}, phone_db='{exist.phone}')"
            )
            return jsonify({
...
def confirm_password(db: Session):
    ...
    try:
        ...
        if "error" in res:
            logger.warning(f"auth.confirm_password: fail (user_id={uid}, error='{res.get('error')}')")
            return jsonify(res), int(res.get("http", 400))
        dt = (perf_counter() - t0) * 1000
...
def account_refresh(acc_id: int, db: Session):
    ...
    def gen():
        ...
        try:
            ...
            if not acc2:
                logger.warning(f"account.refresh.stream: acc not_found (user_id={uid}, acc_id={acc_id})")
            ...
            for ev in iter_refresh_steps_core(db2, acc=acc2, api_id=ap.api_id, api_hash=ap.api_hash):
                if ev.get("stage"):
                    logger.debug(f"account.refresh.stream: stage={ev['stage']} (user_id={uid}, acc_id={acc_id})")
                if ev.get("error"):
                    logger.warning(
                        f"account.refresh.stream: error='{ev.get('error')}' detail='{ev.get('detail')}' "
