...
from backend.domain.admin.repositories import (AdminRepository, AuditLogEntry,
                                               AuditLogRepository, ErrorStats,
                                               SuspiciousActivity, UserInfo)
...
class SQLAlchemyAdminRepository(AdminRepository):
    ...
    def list_users(self) -> list[UserInfo]:
        with unit_of_work_scope(self._session_factory) as session:
            last_login_subq = (
                session.query(
                    AuditLog.user_id, func.max(AuditLog.timestamp).label("last_login")
                )
            ...
            first_log_subq = (
                session.query(
                    AuditLog.user_id, func.min(AuditLog.timestamp).label("created_at")
                )
                .group_by(AuditLog.user_id)
                .subquery()
            )
            ...
            users_with_stats = (
                session.query(
                    User, last_login_subq.c.last_login, first_log_subq.c.created_at
                )
